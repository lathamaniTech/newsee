...
class ProposalInboxRepositoryImpl implements ProposalInboxRepository {
  ...
  searchProposalInbox(LeadInboxRequest req) async {
    try {
      ...
      if (isSuccess) {
        final data =
            responseData[ApiConfig
                .API_RESPONSE_RESPONSE_KEY]['proposalDetails'];
        final totalProposals =
            responseData[ApiConfig
                    .API_RESPONSE_RESPONSE_KEY]['totalRecordCount']
                as int;
        if (data is List) {
          final listOfProposalApplication =
              data
                  .map(
                    (e) =>
                        GroupProposalInbox.fromMap(e as Map<String, dynamic>),
                  )
                  .toList();
          ...
          final listOfProposalApplication = GroupProposalInbox.fromMap(data);
          final proposalInboxResponse = ProposalInboxResponseModel(
            proposalDetails: [listOfProposalApplication],
            totalProposals: totalProposals,
          );
  ...
  getApplicationStatus(req) async {
    try {
      ...
      final isSuccess =
          responseData[ApiConfig.API_RESPONSE_SUCCESS_KEY] == true;
      final Map<String, dynamic> isResponse =
          responseData[ApiConfig.API_RESPONSE_RESPONSE_KEY];

      if (isSuccess || isResponse.isNotEmpty) {
        ApplicationStatusResponse applicationStatus =
            ApplicationStatusResponse.fromMap(isResponse);
  ...
  @override
  Future<AsyncResponseHandler<Failure, ProposalInboxResponseModel>>
  searchPDInbox(PdInboxRequest req) async {
    try {
      String endpoint = ApiConfig.PD_INBOX_API_ENDPOINT;
      final response = await ProposalInboxRemoteDatasource(
        dio: ApiClient().getDio(),
      ).searchProposalInbox(req.toMap(), endpoint);
      ...
      if (isSuccess) {
        final data =
            responseData[ApiConfig.API_RESPONSE_RESPONSE_KEY]['PDDetails'];
        final totalProposals = 20;
      ...
      return AsyncResponseHandler.left(
        HttpConnectionFailure(
          message: "Unexpected Failure during PD Inbox Search",
        ),
      );
    }
  }
